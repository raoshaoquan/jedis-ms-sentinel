...
public class RedisUtil {
    ...
    private Pool<MasterSlaveJedis> pool = null;
    ...
    public RedisUtil(Properties properties) {
        ...
        int minIdle = Integer.valueOf(properties.getProperty("redis.minIdle"));
        int maxIdle = Integer.valueOf(properties.getProperty("redis.maxIdle"));
        int maxWait = Integer.valueOf(properties.getProperty("redis.maxWait"));
        boolean testOnBorrow = Boolean.valueOf(properties.getProperty("redis.testOnBorrow"));
        config.setMaxIdle(maxIdle);
        config.setMinIdle(minIdle);
        config.setMaxTotal(maxWait);
        config.setTestOnBorrow(testOnBorrow);
        String host = properties.getProperty("redis.host");
        Set<String> sentinels = new LinkedHashSet<>();
        sentinels.addAll(Arrays.asList(host.split(",")));
        String masterName = properties.getProperty("redis.master");
        String password = properties.getProperty("redis.pass");
        int timeout = Integer.valueOf(properties.getProperty("redis.timeout"));
        int dbIndex = Integer.valueOf(properties.getProperty("redis.db"));
        ...
        //
        pool = new MasterSlaveJedisSentinelPool(masterName, sentinels, config, timeout, pass, database);
    }

    /**
     * <p>传入ip和端口号构建redis 连接池</p>
     */
    public RedisUtil(String master, Set<String> sentinels,
                     JedisPoolConfig config, int timeout, String password, int dbIndex) {
        String pass = password;
        if (password.trim().equals("")) {
            pass = null;
        }
        this.database = dbIndex;
        pool = new MasterSlaveJedisSentinelPool(master, sentinels, config, timeout, pass, database);
    }
     ...
     *
     */
    public static void returnResource(MasterSlaveJedis jedis) {
        if (jedis != null) {
            jedis.close();
        }
    }
